...
class SectionModel extends AppModel {
  ...
  public function ListItems()
  {
    ...
    $sql = "SELECT t.id_section as id, t.*,
  ...
  private function ListWhere($filter = false)
  {
    ...
    if($this->input->post('filter-active'))
      $sql .= " AND t.active = '1'";
    if($text)
      $sql .= " AND ( t.section like '%{$text}%'  OR  t.title like '%{$text}%'  OR  t.text like '%{$text}%'  OR t.id_section = '{$text}') ";   
    if($this->input->post('filter-id'))
      $sql .= " AND t.id_section = '". $this->input->post('filter-id') ."'";  
    return $sql;
  }  
  ...
  {    
    $sql = "select * from {$this->table} where id_section = '{$id}'";
  ...
  public function SavePost()
  {
    ...
    if( $this->id )
      $sql = $this->db->update_string($this->table, $data, "id_section = '{$this->id}'" );
    else
  ...
  public function DataElement( $id = 0, $null = false)
  {
    ...
    $ret['section'] = $this->input->post() ? $this->input->post('section') : '';
